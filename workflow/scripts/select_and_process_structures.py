...
stcrdab_summary["resolution"] = pd.to_numeric(
    stcrdab_summary["resolution"], errors="coerce"
)
...
for pdb_id in selected_stcrdab["pdb"].unique():
    ...
    for tcr in tcrs:
        ...
        interactions.to_csv(
            os.path.join(output_dir, tcr_name + "_interactions.csv"), index=False
        )
